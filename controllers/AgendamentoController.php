...
class AgendaController extends Controller
{
    ...
    public function actionGetevent()
    {
        if (Yii::$app->request->isAjax) {
            return json_encode(Yii::$app->db->createCommand('SELECT nome, tipo_atendimento, cpf, DATE_FORMAT(horario, "%Y-%m-%dT%H:%i:%s") AS horario, plano_particular, c.id as status, descricao, c.cor FROM agendamento JOIN agenda_status c ON (agendamento.id_status = c.id) WHERE id_agendamento =' . Yii::$app->request->post()['id'])->queryOne());
        }
    }
