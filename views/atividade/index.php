        ...
        'columns' => [    
          ...
          [
            ...
            'value' => function ($data) {
              ...
              $horas_ee = Yii::$app->db->createCommand('SELECT SUM(horas_ee) as horas, SUM(executado_ee) as exe_ee FROM escopo WHERE projeto_id='.$data->id)->queryOne();
              $horas_es = Yii::$app->db->createCommand('SELECT SUM(horas_es) as horas, SUM(executado_es) as exe_es FROM escopo WHERE projeto_id='.$data->id)->queryOne();
              $horas_ep = Yii::$app->db->createCommand('SELECT SUM(horas_ep) as horas, SUM(executado_ep) as exe_ep FROM escopo WHERE projeto_id='.$data->id)->queryOne();
              $horas_ej = Yii::$app->db->createCommand('SELECT SUM(horas_ej) as horas, SUM(executado_ej) as exe_ej FROM escopo WHERE projeto_id='.$data->id)->queryOne();
              $horas_tp = Yii::$app->db->createCommand('SELECT SUM(horas_tp) as horas, SUM(executado_tp) as exe_tp FROM escopo WHERE projeto_id='.$data->id)->queryOne();
              ...
              if($horas!=0){
                $progress = ($horas_ee['exe_ee']+$horas_es['exe_es']+$horas_ep['exe_ep']+$horas_ej['exe_ej']+$horas_tp['exe_tp']) / ($horas) * 100 ;
